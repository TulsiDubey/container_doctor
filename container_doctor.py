...
def stats():
    try:
        ...
        for c in containers:
            ...
            health_status = c.attrs.get('State', {}).get('Health', {}).get('Status', 'healthy')
            if c.status in ["exited", "dead", "created"] or health_status == "unhealthy" or diagnostics.get("status") == "broken":
                status = "broken"
            if status == "healthy":
                healthy += 1
            else:
    ...
    while True:
        ...
        for c in containers:
            ...
            try:
                ...
                health_status = c.attrs.get('State', {}).get('Health', {}).get('Status', 'healthy')
                if c.status in ["exited", "dead", "created"] or health_status == "unhealthy":
                    incident_state[container_name] = {'status': 'broken', 'healthy_checks': 0}
                    logger.warning(f"Detected unhealthy/broken status {c.status}/{health_status} for {container_name}")
                    ...
                    if logs:
                        ...
                        if error_patterns:
                            ...
                            if diagnosis:
                                ...
                                send_slack_alert(container_name, diagnosis, extra="Crash Diagnostic")
